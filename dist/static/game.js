...
function ensureSkinMotionTrail(mesh, entity) {
    ...
    const galaxyClouds = skinId === 'solar' ? [
        { color:0x244bd0, x:-.32, z:2.15, scale:[1.18,3.55], turn:-.16, opacity:.24 },
        { color:0x7034c7, x:.32, z:2.20, scale:[1.18,3.55], turn:.16, opacity:.25 },
        { color:0x9b4fd8, x:0, z:2.06, scale:[.92,3.10], turn:0, opacity:.18 }
    ].map((spec, index) => {
    ...
    if (!flying && mesh.userData.legs) mesh.userData.legs.forEach((leg, index) => {
        ...
        if (mesh.userData.skinMotionTrail) {
            ...
            trail.galaxyClouds?.forEach((cloud, index) => {
                ...
                cloud.rotation.set(-Math.PI / 2, cloud.userData.baseTurn + Math.sin(phase * 1.1 + index) * .035, 0);
            ...
            trail.glitters.forEach((sparkle, index) => {
                ...
                if (trail.id === 'solar') {
                    ...
                    sparkle.position.set(Math.sin(crossing) * (.3 + progress * .65), .36 + Math.cos(phase * 2.9 + index) * .16, .48 + progress * 3.5);
...
function openAccountPanel(kind) {
    ...
    if (kind === 'hero') {
        ...
        content.innerHTML = `<div class="feedback-box"><div class="feedback-heading">v3.5.12 · 皮肤碎片与局外宝箱</div><div>更新时间：2026 年 8 月 11 日</div></div><div class="skill-card"><div class="skill-name">🎨 新皮肤</div><div class="skill-desc">• 新增史诗皮肤：小狐狸「月影灵狐」。<br>• 新增神话皮肤：鲨鱼「星海巨鲨」。<br>• 新增传说皮肤：狮子「烈阳狮王」。<br>• 三款皮肤均可直接用金币购买，也可通过对应品质碎片兑换。</div></div><div class="skill-card"><div class="skill-name">🧩 碎片兑换</div><div class="skill-desc">• 背包新增普通、史诗、神话、传说皮肤碎片。<br>• 商城新增“碎片兑换”页，位于皮肤与道具之间，按品质分区展示可兑换皮肤。<br>• 史诗皮肤需 20 碎片 / 10000 金币；神话需 35 碎片 / 15000 金币；传说需 50 碎片 / 25000 金币。</div></div><div class="skill-card"><div class="skill-name">🎁 局外宝箱掉落</div><div class="skill-desc">• 普通、稀有宝箱可获得普通碎片，或少量史诗碎片。<br>• 史诗宝箱可获得史诗碎片，或少量神话碎片；神话宝箱可获得神话碎片，或少量传说碎片。<br>• 传说宝箱可获得传说碎片；若降档则会获得更多对应品质碎片。</div></div><div class="skill-card"><div class="skill-name">📅 百天签到</div><div class="skill-desc">• 百天签到第 50、75、100 天改为赠送局外宝箱券。<br>• 宝箱券会放进背包，在当天免费宝箱领取后可额外开启一轮完整宝箱挑战。</div></div>`;
